...
defmodule Reactor.File.Ops do
  ...
  @doc "An error wrapped version of `File.touch/2`"
  def touch(path, time, step, message \\ "Unable to touch file") do
    with {:error, reason} when FileError.is_posix(reason) <- File.touch(path, time) do
      {:error,
       FileError.exception(
         action: {:touch, time},
         step: step,
